...
// V4.1 - Modificações Solicitadas: Queda Suave + Novo Título + Tela Game Over
// =========================================================
...
// ALTERAÇÃO 1: Gravidade REDUZIDA para queda mais SUAVE
const rocketGravidade = 0.2; 
...
        // ALTERAÇÃO 1: Limite máximo de queda (para 5) reduzido para queda mais suave
        rocketVelocidade = constrain(rocketVelocidade, -12, 5);
...
    }
    // Adicionada a reinicialização com ESPAÇO
...
    fill(255, 255, 255); 
    // ALTERAÇÃO 2: Nova frase de título
...
// ALTERAÇÃO 3: Tela de Game Over Aprimorada
function drawGameOverScreen() {
    push();
    // Título de Game Over
    textSize(60 * (width / 800)); 
    fill(255, 50, 50); 
    text("MÁQUINA DESTROÇADA!", width / 2, height / 2 - 100);
    
    // Motivo da Perda
    textSize(30 * (width / 800));
    fill(255, 255, 0);
    text(`Motivo: ${motivoFimDeJogo}`, width / 2, height / 2 - 40);
    
    // Pontuação Final (Em destaque)
    textSize(36 * (width / 800));
    fill(255);
...
    textSize(24 * (width / 800));
    fill(255, 200, 0);
    text(`RECORDE ATUAL: ${highScore}`, width / 2, height / 2 + 70);

    // Botão / Instrução de Reinício (Em destaque)
    textSize(24 * (width / 800));
    fill(100, 255, 100); // Verde para "Botão" de Recomeço
    text("Toque/Clique ou ESPAÇO para TENTAR DE NOVO", width / 2, height / 2 + 130);
    pop();
}
